...
class Form{
	...
	function Label(){
		echo '<label class="'.$this->class.'" id="label_'.$this->id.'" for="'.$this->name.'">'.$this->title;
	...
	function Input(){
		...
		$typeList = array('text','radio','submit','captcha','password','checkbox','hidden', 'button');
		$attributList =array(
		...
		if(!in_array($this->type, $typeList)){
			...
			if($this->type == "captcha"){
				echo '<img style="margin-top:2px;" src="./poo/formulaire/math_captcha/image.php" alt="'.$this->name.'" title="'.$this->name.'" id="captcha_im" /><br />';
				$this->type = "texte";
			}
				...
				switch($condition0){
					...
					case 'isNotEmpty':
						if(!Algorithms::isNotEmpty($value)){
							$this->validated[]=0;
							return "<li>Le champs <em>".$_SESSION[$name]['fieldLabel']."</em> est obligatoire.</li>"; 
					...
					case 'isEmail':
						if(!Algorithms::isEmail($value)){ 
							$this->validated[]=0;
							return "<li>Votre e-mail n'est pas valide.</li>";
						}else{
							$this->validated[]=1;
						}
						break;
					...
					case 'isDomain':
						if(!Algorithms::isDomain($value)){
							$this->validated[]=0;
							return "<li>Le champs <em>".$_SESSION[$name]['fieldLabel']."</em> contient un domaine invalide.</li>";
					...
					case 'isNumeric':
						if(!Algorithms::isNumeric($value)){
							$this->validated[]=0;
							return "<li>Le champs <em>".$_SESSION[$name]['fieldLabel']."</em> doit &ecirc;tre de type Num&eacute;rique.</li>";
					...
					case 'isString':
						if(!Algorithms::isString($value)){
							$this->validated[]=0;
							return "<li>Le champs <em>".$_SESSION[$name]['fieldLabel']."</em> doit contenir des lettres.</li>";
						}else{
							$this->validated[]=1;
						}
						break;	
					...
					case 'isDate':
						if(!Algorithms::isDate($value)){
							$this->validated[]=0;
							return "<li>Le champs <em>".$_SESSION[$name]['fieldLabel']."</em> doit &ecirc;tre une date.</li>";
					...
					case 'isStringOnly':
						if(!Algorithms::isStringOnly($value)){
							$this->validated[]=0;
							return "<li>Le champs <em>".$_SESSION[$name]['fieldLabel']."</em> ne doit pas contenir de chiffre(s).</li>";
					...
					case 'minDigit':
						if(!Algorithms::minDigit($value,$condition1)){
							$this->validated[]=0;
							return "<li>Le champs <em>".$_SESSION[$name]['fieldLabel']."</em> doit avoir au minimum ".$condition1." catact&egrave;re(s).</li>";
					...
					case 'maxDigit':
						if(!Algorithms::maxDigit($value,$condition1)){
							$this->validated[]=0;
							return "<li>Le champs <em>".$_SESSION[$name]['fieldLabel']."</em> ne doit pas avoir plus de ".$condition1." catact&egrave;re(s).</li>";
					...
					case 'nbDigit':
						if(!Algorithms::nbDigit($value,$condition1)){
							$this->validated[]=0;
							return "<li>Le champs <em>".$_SESSION[$name]['fieldLabel']."</em> doit avoir exactement ".$condition1." caract&egrave;re(s).</li>";
					...
					case 'captcha':
						if(!Algorithms::Captcha($value)){
							$this->validated[]=0;
							return "<li>Le champs <em>".$_SESSION[$name]['fieldLabel']."</em> n'a pas le bon r&eacute;sultat.</li>";
						...
						break;
					default:
						return "<li>La condition [<em>".$condition0."</em>] n'existe pas dans la class Validation.</li>";
				}
			}
		}
	}
